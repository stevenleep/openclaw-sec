...
整个过程中，每一步都通过了 OpenClaw 的工具审批——因为 Agent 确实在"完成任务"。但数据已经泄露了。

**OpenClaw 的沙箱和工具策略解决的是"Agent 能不能做"的问题。本项目解决的是"Agent 做了之后，信息有没有泄露"的问题。**
...
Agent 在回复中无意包含 API Key、数据库凭证、用户 PII 等敏感信息。这些信息可能来自 Agent 读取的文件、环境变量或工具返回值。OpenClaw 的工具审批发生在执行前，无法检查执行后的输出内容。
...
安全事件发生后需要回答：什么时间、哪个会话、Agent 读了什么、发了什么、决策依据是什么。OpenClaw 的基础命令日志不包含内容扫描结果、调用链分析上下文和风险判定信息。
